...
ctrl_error = roe_desired' - qns_osc_roe_norm; % desired state - actual state
...
nav_error = (qns_osc_roe - qns_roe_ekf) * sma_c; % reference truth - state
...
% Formal navigation error statistics
nav_err_std_formal = sqrt(P_diags(end,:));

%% Map the formal navigation error to RTN
nav_stdf_rtn = zeros(steps,6);
for j=1:steps
    % Convert ROEs to singular, singular to ECI pos/vel
%     [r_ECI,v_ECI] = singular_oe2rv(mu,roe2singular(oe_init_c,sqrt(P_diags(j,:)) )); 
%     % Convert to RTN pos/vel
%     [r_RTN,v_RTN] = absToRelState(mu,[r_init_c;v_init_c],[r_ECI;v_ECI]);
%     nav_stdf_rtn(j,:) = [r_RTN,v_RTN];
    nav_stdf_rtn(j,:) = roe2RTN(sqrt(P_diags(j,:))/sma_c,qns_oe_c(j,:),mu); 
end

figure('Name','RTN Navigation Error'); hold on
plot_rtn_time_series(orbit_periods(ss_idx:end),nav_stdf_rtn(ss_idx:end,1:3),rtn_labels);
...
STATS_NAV = table(nav_err_true_mean',nav_err_true_std',nav_err_std_formal',mean_prefit',std_prefit',mean_postfit',std_postfit',...
    'VariableNames',["Nav Error True Mean", "Nav Error True Std", "Nav Error Formal Std", "Prefit Mean Residuals", "Prefit Std Residuals", "Postfit Mean Residuals", "Postfit Std Residuals"])
