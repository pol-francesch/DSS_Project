...
eci_c = eci_c(1:end-1,:);
eci_d = eci_d(1:end-1,:);
qns_oe_c = qns_oe_c(1:end-1,:); % osculating QNS [a,ex,ey,i,Om,u]
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compute delta-V lower bounds
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Theoretical lower bound
d_roe_th = roe_init2 - roe_init1; % non-dimensionalized
d_inc_th = norm(d_roe_th(5:6));
d_ecc_th = norm(d_roe_th(3:4));

% In-plane
dv_LB_ip_th = sma_c*n*d_ecc_th/(2*eta)

% Out-of-plane
dv_LB_oop_th = sma_c*n*(1-ecc_c)/eta * d_inc_th

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Actual change in ROEs achieved
d_roe_actual = qns_roe(end,:)'-roe_init1; % non-dimensionalized
d_inc_act = norm(d_roe_actual(5:6));
d_ecc_act = norm(d_roe_actual(3:4));

% In-plane
dv_LB_ip_act = sma_c*n*d_ecc_act/(2*eta)

% Out-of-plane
dv_LB_oop_act = sma_c*n*(1-ecc_c)/eta * d_inc_act
...
function P = build_P(roe, roe_ref, oe_c, N, K)
    droe = roe - roe_ref;
    % phi_ip = atan2(roe(4),roe(3)); % "Delta delta e": variation in e across the reconfig.
    % phi_oop = atan2(roe(6), roe(5));
