...
delta_us = [0, pi, 2*pi, 3*pi]+2*pi; % location differences (mean arg. of lat) between maneuvers
t_man = [0, T/2, T, 3/2*T]+T; % time of maneuvers

% u_ks = [0, pi/2, pi, 3*pi/2];
% delta_us = [0, pi/2, pi, 3*pi/2] + 2*pi;
% t_man = [0, T/4, T/2, 3*T/2]+T;
...
[state_eci_c, state_eci_d] = piecewise_dfq(rv_init_c, rv_init_d,dV, t_man, options, tspan, stepSize,mu,rE,J2);
...
function plot_roe_state_w_init_and_final(roe_set, roe_init, roe_final)
    ...
    plot(roe_set(:,3),roe_set(:,4),'.','Color',"#0072BD",'MarkerSize',20);
    plot(roe_init(3), roe_init(4), '.', 'Color',"#EDB120",'MarkerSize', 20);
    plot(roe_final(3), roe_final(4), 'r.', 'MarkerSize', 20);
    hold on
    ...
    plot(roe_set(:,5),roe_set(:,6),'.','Color',"#0072BD",'MarkerSize',20); hold on
    plot(roe_init(5), roe_init(6), '.','Color',"#EDB120", 'MarkerSize', 20);
    plot(roe_final(5), roe_final(6), 'r.', 'MarkerSize', 20);
    hold on
    ...
    plot(roe_set(:,2),roe_set(:,1),'.','Color',"#0072BD",'MarkerSize',20); hold on
    plot(roe_init(2), roe_init(1), '.','Color',"#EDB120", 'MarkerSize', 20);
    plot(roe_final(2), roe_final(1), 'r.', 'MarkerSize', 20);
    hold on
    xlabel('$a \delta \lambda $ (m)')
    ylabel('$a \delta a$ (m)')
    legend('Time history','Initial', 'Target')
end
