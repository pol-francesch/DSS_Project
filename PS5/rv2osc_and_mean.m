function [osc_oe,mean_oe,osc_roe,mean_roe] = rv2osc_and_mean(mu, rE, J2, J2_flag, deg_flag, chief, deputy)
    ...
    % osc_roe = singualaroe2qnsroe(osc_qns_oe_chief, osc_qns_oe_deputy);
    % mean_roe = singualaroe2qnsroe(mean_qns_oe_chief, mean_qns_oe_deputy);
...
function roe = singualaroe2qnsroe(oe_c, oe_d)
    % oe_c = [sma, e, inc, raan, aop, ta / M] 
    % Singular OEs to QNS ROEs [sma, dlambda, dex, dey, dix, diy]
    roe = [(oe_d(1) - oe_c(1)) / oe_c(1);
           (oe_d(6) + oe_d(5)) - (oe_c(6) + oe_c(5)) + (oe_d(4) - oe_c(4)) / cos(oe_c(3));
           oe_d(2)*cos(oe_d(5)) - oe_c(2)*cos(oe_c(5));
           oe_d(2)*sin(oe_d(5)) - oe_c(2)*sin(oe_c(5));
           oe_d(3) - oe_c(3);
           (oe_d(4) - oe_c(4))*sin(oe_c(3))]';
end