...
for j=1:steps
    ...
    droe = roe - roe_ref;
    droe_reduced = [droe(1), droe(3:6)];
    u_reduced = (-pinv(B)*(A*droe_reduced' + P*droe_reduced'))'; % Keplerian A is all 0s if we remove dlambda!
    ...
    delta_de = norm(roe_ref(3:4)-roe_init2(3:4)); % TODO: check that this is the reference objective for the current step
    delta_di = norm(roe_ref(5:6)-roe_init2(5:6)); % updates every step
    deltaV_ip = sma_c*n_c*delta_de/(2*eta_c); % optimal in-plane delta-V
    deltaV_oop = sma_c*n_c*(1-oe_c(2))/(eta_c)*delta_di; % optimal out-of-plane delta-V
    
    u_d_ip = 2*deltaV_ip/(T*num_const_orbs)*prod_seq;
    u_d_oop = 2*deltaV_oop/(T*num_const_orbs)*prod_seq;
    u_d = norm([u_d_ip, u_d_oop]);
    rho_e = (droe_reduced/norm(droe_reduced))'; % tracking error direction
    gamma_time = .5*(u_d/norm( pinv(B)*P*rho_e ))^2;
    V = .5*(droe_reduced)*droe_reduced';
    ...
    roe_ref = roe_ref + refgov_update*stepSize;
    ...
    roe_ref(1) = roe_ref(1) - 2/3*dot_dlambda / n_c;
